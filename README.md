...
- **Header & Footer** - SfUserAuth component generates both the header and footer view
...
- **Search Input** - SfUserAuth ships with a search input field
- **Social Media** - SfUserAuth accepts social media links as an un-ordered list and renders them into the footer
- **Copyright Notice** - SfUserAuth accepts copyright notice as input and renders it into the footer
...
- **Call-to-action Button** - SfUserAuth comes with a call-to-action button, which can be used to highlight key actions such as sign in and subscribe.
...
- **Customizability** - SfUserAuth colors can be customized using css custom properties
- **Responsiveness** - SfUserAuth nicely adapts to all screen sizes
...
<html>
  <head>
    ...
    <!-- Integration material icons as they are used in sf-user-auth -->
    ...
    <!-- Integration lit and sf-user-auth web components module -->
    <script type="module">
  ...
  <body style="margin: 0px;">
    ...
    <script>
      document.getElementsByTagName('sf-user-auth')[0].addEventListener('searchClick', () => {console.log('hurray');})
      document.getElementsByTagName('sf-user-auth')[0].addEventListener('routeChange', (e) => {
...
Routing is enabled by default. Every hash link specified in the SfUserAuth component needs to be backed by its separate html component. For example the links in the brand info section (above section), contains #home as the link. For routing to work, simply create a separate home.html component for it. SfUserAuth will then automatically take care of routing.
...
Search Input is shown by default and the SfUserAuth component throws a searchClick event after the user types something in the search input field and presses enter. To handle the searchClick event:

```html

    <script>
      document.getElementsByTagName('sf-user-auth')[0].addEventListener('searchClick', () => {console.log('search clicked');})
    </script>
