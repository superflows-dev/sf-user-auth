...
const isLegacyBrowser_1 = require("./isLegacyBrowser");
const babelTransform_1 = require("./babelTransform");
const injectPolyfillsLoader_1 = require("./injectPolyfillsLoader");
...
function legacyPlugin(options = {}) {
    ...
    return {
        ...
        transformCacheKey(context) {
            return isLegacyBrowser_1.isLegacyBrowser(context, logger) ? 'legacy' : undefined;
        },
        async serve(context) {
            if (!isLegacyBrowser_1.isLegacyBrowser(context, logger)) {
                return;
            }
            /**
             ...
             * serve
             */
            if (dev_server_core_1.isInlineScriptRequest(context.url)) {
        ...
        async transform(context) {
            ...
            if (context.response.is('js')) {
                ...
                const config = context.URL.searchParams.get(constants_1.PARAM_TRANSFORM_SYSTEMJS) === 'true'
                    ? babelTransform_1.systemJsConfig
                    : babelTransform_1.es5Config;
                const filePath = dev_server_core_1.getRequestFilePath(context.url, rootDir);
                const transformed = await babelTransform_1.babelTransform(filePath, context.body, config);
                context.body = transformed;
                return;
            }
            if (context.response.is('html')) {
                const result = await injectPolyfillsLoader_1.injectPolyfillsLoader(context, options.polyfills);
        ...
        transformImport({ source, context }) {
            ...
            return `${source}${prefix}${constants_1.PARAM_TRANSFORM_SYSTEMJS}=true`;
        },
    };
}
