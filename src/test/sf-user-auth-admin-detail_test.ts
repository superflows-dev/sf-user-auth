...
suite('sf-user-auth > Admin tests', () => {
    ...
    test('admin tests', async () => {
        ...
        const h1 = el.shadowRoot!.querySelectorAll('h1')[0]! as HTMLElement;
        assert.ok(h1.outerHTML.indexOf('User Information') >= 0);

        const locked = el.shadowRoot!.querySelectorAll('#locked')[0]! as HTMLElement;
        locked.dispatchEvent(clickEvent);

        await new Promise((r) => setTimeout(r, TIMEOUT));

        // const ipReason = el.shadowRoot!.querySelectorAll('#reason')[0]! as HTMLInputElement;
        // ipReason.value = 'abcasd asdasd';
        // ipReason.dispatchEvent(new KeyboardEvent('keyup', {'key': 'm'}));

        // await new Promise((r) => setTimeout(r, TIMEOUT));
        
        const ipName = el.shadowRoot!.querySelectorAll('#name')[0]! as HTMLInputElement;
        assert.ok(ipName.outerHTML.indexOf('disabled') < 0);
        console.log('ipname', ipName.value)
        assert.ok(ipName.value.indexOf('Admin') >= 0); 
        
        const unlocked = el.shadowRoot!.querySelectorAll('#unlocked')[0]! as HTMLElement;
        unlocked.dispatchEvent(clickEvent);
