...
suite('sf-user-auth > Admin tests', () => {
    ...
    test('refresh tests', async () => {
        ...
        const badgeAdmin = el.shadowRoot!.querySelectorAll('.badge')[0]! as HTMLInputElement;
        assert.ok(badgeAdmin.outerHTML.indexOf('Admin') >= 0);

        const submit = el.shadowRoot!.querySelectorAll('.submit-search')[0]! as HTMLElement;
        assert.ok(submit.outerHTML.indexOf('disabled') > 0);

        const ipSearch = el.shadowRoot!.querySelectorAll('.input-search')[0]! as HTMLInputElement;

        ipSearch.value = 'abc@gmail.co';
        ipSearch.dispatchEvent(new KeyboardEvent('keyup', {'key': 'm'}));
