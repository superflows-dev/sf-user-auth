...
suite('sf-user-auth > Basic tests', () => {
    ...
    test('verify tests', async () => {
        ...
        const resend = el.shadowRoot!.querySelectorAll('.resend')[0]! as HTMLLinkElement;

        resend.dispatchEvent(clickEvent);

        await new Promise((r) => setTimeout(r, TIMEOUT));

        const success = el.shadowRoot!.querySelectorAll('.div-row-success-message')[0]! as HTMLDivElement;

        assert.ok(success.innerHTML.indexOf('success') >= 0); 
