...
suite('sf-user-auth > Basic tests', () => {
    ...
    test('sign up render error', async () => {
        ...
        const ipName = el.shadowRoot!.querySelectorAll('#name')[0]! as HTMLInputElement;
        const errorName = el.shadowRoot!.querySelectorAll('#error-client-name')[0]!;

        ipName.dispatchEvent(new KeyboardEvent('keyup', {'key': 'h'}));
        await new Promise((r) => setTimeout(r, TIMEOUT));
        assert.ok(errorName.outerHTML.indexOf('display: block') >= 0); 

        ipName.value = 'hrushi m';
        ipName.dispatchEvent(new KeyboardEvent('keyup', {'key': 'm'}));
        
        await new Promise((r) => setTimeout(r, TIMEOUT));
        assert.ok(errorName.outerHTML.indexOf('display: none') >= 0); 

        const ipEmail = el.shadowRoot!.querySelectorAll('#email')[0]! as HTMLInputElement;
        const errorEmail = el.shadowRoot!.querySelectorAll('#error-client-email')[0]!;

        ipEmail.dispatchEvent(new KeyboardEvent('keyup', {'key': 'h'}));
        await new Promise((r) => setTimeout(r, TIMEOUT));
        assert.ok(errorEmail.outerHTML.indexOf('display: block') >= 0); 

        ipEmail.value = 'h@abc.co';
        ipEmail.dispatchEvent(new KeyboardEvent('keyup', {'key': 'm'}));
        
        await new Promise((r) => setTimeout(r, TIMEOUT));
        assert.ok(errorEmail.outerHTML.indexOf('display: none') >= 0); 

        const ipTerms = el.shadowRoot!.querySelectorAll('#terms')[0]! as HTMLInputElement;
        ipTerms.checked = true;

        await new Promise((r) => setTimeout(r, TIMEOUT));

        const ipPrivacy = el.shadowRoot!.querySelectorAll('#privacy')[0]! as HTMLInputElement;
        ipPrivacy.checked = true;
        ...
        await new Promise((r) => setTimeout(r, TIMEOUT));
        const divRowError = el.shadowRoot!.querySelectorAll('.div-row-error-message')[0]! as HTMLDivElement;
        assert.ok(divRowError.innerHTML.indexOf('already exists') >= 0);
